module ReductionUserProfilesHelper
  ...
  #今日使った金額
  def used_money
    ((current_user.reduction_user_profile.tabaco_price / 20.0) * current_user.reduction_user_profile.smoking_count).floor
  end
  ...
  def used_time_final
    (Time.parse("1/1") + (used_time_second - used_time_day * 86400)).strftime("#{used_time_day}日%H時間%M分%S秒")
  end
  ...
  def lost_life_final
    (Time.parse("1/1") + (lost_life_second - lost_life_day * 86400)).strftime("#{lost_life_day}日%H時間%M分%S秒")
  end
  
  def lost_wage
    ((used_time_second / 3600.0) * current_user.reduction_user_profile.hourly_wage).floor
  end
